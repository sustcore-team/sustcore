...
namespace cap {
     ...
     */
    using CTreeLink = tay::compact_intrusive_tree_hook<Capability>;
     ...
     *
     * `object` 通过单指针 `KObjectRef` 对目标持有一个对象强引用。`deriv` 将副本和 mint 结果接入
    ...
    struct alignas(CAPABILITY_ALIGNMENT) Capability {
        Capability(KObject &object, u64_t rights, u64_t badge, u32_t generation) noexcept
            : header{.flags = cell_kind_flags(CellKind::CAPABILITY),
                     .aux   = static_cast<u16_t>(object.type()),
                     .data  = generation},
              object(object),
              rights(rights),
              badge(badge) {}
    ...
    using CTree =
        tay::intrusive_tree<Capability,
                            tay::locate_member<Capability, CTreeLink, &Capability::deriv>>;
    ...
    public:
         ...
         */
        [[nodiscard]] static tay::expected<CNode *, CError> create_pages(
         ...
         */
        [[nodiscard]] static tay::expected<CNode *, CError> create_small() noexcept;
        ...
        using CellRef = tay::variant<Capability *, CNodeMetadata *, FreeSlot *>;
        using ConstCellRef =
            tay::variant<const Capability *, const CNodeMetadata *, const FreeSlot *>;

        [[nodiscard]] CellRef cell(u16_t slot) noexcept;
        [[nodiscard]] ConstCellRef cell(u16_t slot) const noexcept;
    ...
    private:
        ...
        void drop_cell_locked(u16_t slot) noexcept;
         ...
         * @pre 调用方持有 `lock_`。
         */
        [[nodiscard]] tay::expected<u16_t, CError> reserve_locked(
         ...
         */
        void publish_locked(u16_t slot, KObject &object, u64_t rights, u64_t badge,
         ...
         */
        [[nodiscard]] KObjectRef<KObject> erase_locked(u16_t slot, bool reparent_children) noexcept;

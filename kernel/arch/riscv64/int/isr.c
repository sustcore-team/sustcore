                       ...
                       InterruptContextRegisterList *reglist_ptr)
{
    ...
    if (scause.cause < sizeof(exception_msg)/sizeof(exception_msg[0])) {
        log_info("发生异常! 类型: %s (%lu)", exception_msg[scause.cause], scause.cause);
    } else {
        log_info("发生异常! 类型: 未知 (%lu)", scause.cause);
    }

    // 输出寄存器状态
    log_info("scause: 0x%lx, sepc: 0x%lx, stval: 0x%lx", scause.value, sepc, stval);
    ...
    for (int i = 1; i < 8; i ++) {
        log_info(
            ...
            i * 4,     reglist_ptr->regs[i * 4 - 1],
            i * 4 + 1, reglist_ptr->regs[i * 4 + 0],
            i * 4 + 2, reglist_ptr->regs[i * 4 + 1],
            i * 4 + 3, reglist_ptr->regs[i * 4 + 2]
        );
    }

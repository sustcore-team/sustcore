...
void default_isr(void) {
    ...
    if (scause.interrupt) {
        INTERRUPT.INFO("默认处理程序: 这是一个中断(Vectored模式)");
    } else {
        INTERRUPT.INFO("默认处理程序: 这是一个异常(Vectored模式)");
    }
...
void general_isr(void) {
    ...
    if (scause.interrupt) {
        INTERRUPT.INFO("通用处理程序: 这是一个中断(Vectored模式)");
    } else {
        general_exception(scause, sepc, stval, ctx);
    }
...
void timer_isr(void) {
    ...
    // TODO: 计算时间间隔
    timer_handler(scause, sepc, stval, ctx);
...
void test(void) {
    ...
    if (scause.interrupt) {
        INTERRUPT.INFO("这是一个TEST:中断:TIMER");
    } else {
        INTERRUPT.INFO("这是一个TEST:异常:TIMER");
        general_exception(scause, sepc, stval, ctx);
    }
...
void primary_isr(void) {
    ...
    if (scause & RISCV_CPU_INTERRUPT_MASK) {
        INTERRUPT.INFO("这是一个中断(Direct模式)");
        if (scause.cause == 5) {
            // 计时器中断
            timer_handler(scause, sepc, stval, ctx);
        } else {
            INTERRUPT.INFO("无对应中断解决方案: 0x%lx", scause.cause);
        }
    } else {
        general_exception(scause, sepc, stval, ctx);
    }
...
void init_ivt(void) {
    ...
    // 输出调试信息
    INTERRUPT.DEBUG("IVT 地址: 0x%lx", (umb_t)IVT);
#elif IVT_MODE == DIRECT
    csr_stvec_t stvec;
    ...
    INTERRUPT.DEBUG("primary_isr 地址: 0x%lx", (umb_t)primary_isr);
#endif

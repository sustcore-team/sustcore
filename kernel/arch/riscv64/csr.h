...
typedef union {
    ...
    struct {
        ...
        umb_t  sum   : 1;      // sstatus[18] SUM Supervisor User Memory Access (允许S-MODE访问U-MODE内存)
        umb_t  mxr   : 1;      // sstatus[19] MXR Make eXecutable Readable (可执行页可读)
        umb_t  wrpi5 : 3;      // sstatus[20:22] WRPI
        umb_t  spelp : 1;      // sstatus[23] SPELP Supervisor Previous(SP) Expected Landing Pad(ELP) (预期落地点)
        umb_t  sdt   : 1;      // sstatus[24] SDT S-Mode Disable Trap (禁用S-MODE陷阱) 控制double-trap. 只在SDT = 0时发送中断并令SDT = 1.
                               // 当且仅当sstatus.sdt = 0时, 会deliver一个trap, 并在此时令sstatus.sdt = 1
                               // 此时sstatus.sie自动清零 ; 调用sret则将sstatus.sie恢复到sstatus.spie的值, 将sstatus.sdt复位为0.
        umb_t  wrpi6 : 7;      // sstatus[25:31] WRPI
        umb_t  uxl   : 2;      // sstatus[32:33] UXL[1:0] User XLEN (用户模式下的XLEN)
        umb_t wrpi7 : 29;      // sstatus[34:62] WRPI
        umb_t  sd    : 1;      // sstatus[63] SD Supervisor Dirty (脏状态) 说明是否有任一扩展处于dirty状态
    } PACKED;
} csr_sstatus_t;
...
// 当对应位被设置后, 如果满足条件(例如SIE寄存器中对应的位也被设置), 则会触发相应的中断
 ...
 */
static inline csr_scounteren_t csr_xchg_scounteren(csr_scounteren_t scounteren) {

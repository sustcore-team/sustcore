...
FDTNodeDesc get_sub_device(const FDTDesc *fdt, FDTNodeDesc parent,
                           const char *name) {
    FDTNodeDesc child;
    // 遍历并匹配
...
FDTPropDesc get_device_property(const FDTDesc *fdt, FDTNodeDesc node,
                                const char *name) {
    FDTPropDesc prop;
    // 遍历并匹配
...
const char *get_device_property_value_as_string(const FDTDesc *fdt,
                                                FDTPropDesc prop) {
...
int get_reg_region_number(const FDTDesc *fdt, FDTPropDesc prop, int addr_cells,
                          int size_cells) {
    FDTPropVal val       = get_device_property_value(fdt, prop);
...
int get_device_property_value_as_reg_regions(const FDTDesc *fdt,
                                             FDTPropDesc prop, int addr_cells,
                                             int size_cells, FDTRegVal *regions,
                                             int max_regions) {
    FDTPropVal val       = get_device_property_value(fdt, prop);
    ...
    for (int i = 0; i < num_regions; i++) {
        ...
        for (int j = 0; j < addr_cells; j++) {
            address =
                (address << 32) | fdt32_to_cpu(cells[i * cells_per_region + j]);
        }
...
static void print_property_value(const char *name, const void *value, int len) {
    // 根据属性名称和长度判断类型并打印
    if (strcmp(name, "compatible") == 0 || strcmp(name, "model") == 0 ||
        strcmp(name, "status") == 0 || strcmp(name, "name") == 0 ||
...
static void print_reg_property(const char *name, int addr_cells, int size_cells,
                               FDTPropVal prop_reg, int depth) {
    int cells_per_region = addr_cells + size_cells;
    int bytes_per_region = cells_per_region * sizeof(fdt32_t);
...
// 递归打印节点及其所有属性
static void print_node_recursive(const FDTDesc *fdt, FDTNodeDesc node,
                                 int depth) {
    ...
    if (prop_regs != -1) {
        ...
        if (parent >= 0) {
            ...
            print_reg_property("reg", addr_cells, size_cells,
                               get_device_property_value(fdt, prop_regs),
                               depth + 1);
        }
    }
...
// 统计函数
static void count_nodes_props(const FDTDesc *fdt, FDTNodeDesc node, int *nodes,
                              int *props) {
    (*nodes)++;
...
void print_device_tree_detailed(const FDTDesc *fdt) {
    ...
    while (fdt_get_mem_rsv(fdt, i, &address, &size) >= 0) {
        if (address == 0 && size == 0)
            break;
        kprintf("  保留区域 %d: 0x%016" PRIx64 " - 0x%016" PRIx64
                " (大小: 0x%" PRIx64 " 字节)\n",
    ...
    count_nodes_props(fdt, get_device_root(fdt), &total_nodes,
                      &total_properties);
    kprintf("  总节点数: %d\n", total_nodes);
    kprintf("  总属性数: %d\n", total_properties);
    kprintf("\n");

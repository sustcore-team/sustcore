...
namespace FDTHelper {
    ...
    static void print_reg_property(const char *name, int addr_cells,
                                   int size_cells, PropVal prop_reg,
                                   int depth) {
        int cells_per_region = addr_cells + size_cells;
        int bytes_per_region = cells_per_region * sizeof(fdt32_t);
    ...
    static void print_node_recursive(const FDTDesc *fdt, FDTNodeDesc node,
        ...
        fdt_for_each_property_offset(prop, fdt, node) {
            ...
            prop_value =
                fdt_getprop_by_offset(fdt, prop, &prop_name, &prop_len);
        ...
        FDTPropDesc prop_regs = get_property(node, "reg");
        if (prop_regs != -1) {
            ...
            if (parent >= 0) {
                ...
                print_reg_property("reg", addr_cells, size_cells,
                                   get_property_value(prop_regs), depth + 1);
            }
        }
    ...
    // 统计函数
    static void count_nodes_props(const FDTDesc *fdt, FDTNodeDesc node,
                                  int *nodes, int *props) {
        (*nodes)++;
    ...
    void print_entire_device_tree(void) {
        ...
        // 打印根节点及其所有子节点
        print_node_recursive(fdt, get_root_node(), 0);
    }
    ...
    void print_device_tree_detailed(void) {
        ...
        count_nodes_props(fdt, get_root_node(), &total_nodes,
                          &total_properties);
        kprintf("  总节点数: %d\n", total_nodes);
        kprintf("  总属性数: %d\n", total_properties);
        kprintf("\n");

...
namespace kernel::timer {
    ...
    enum class TimerCancelResult : u8_t {
    ...
    /**
     * @brief 借用 PrecisionTimerNode 和 typed Worklet 的 BSP precision timer engine。
     *
     ...
     */
    class PrecisionTimerEngine final {
    public:
        constexpr PrecisionTimerEngine() noexcept                     = default;
        PrecisionTimerEngine(const PrecisionTimerEngine &)            = delete;
        PrecisionTimerEngine &operator=(const PrecisionTimerEngine &) = delete;

        /** @brief 绑定固定生命期的 BSP deadline coordinator，只能调用一次。 */
        void initialize(DeadlineState &deadlines) noexcept;
         ...
         */
        [[nodiscard]] tay::expected<void, TimerError> arm(PrecisionTimerNode &node,
                                                          units::time deadline,
         ...
         * 转换，不表示 Worklet 已经执行。
         */
        [[nodiscard]] TimerCancelResult cancel(PrecisionTimerNode &node) noexcept;
        ...
        void retire(PrecisionTimerNode &node) noexcept;

        /** @brief 将 RETIRED node 清空为 IDLE，使宿主可以销毁或重新 arm。 */
        void reset(PrecisionTimerNode &node) noexcept;

        [[nodiscard]] hal::CpuClockDeadline root_deadline() noexcept;
        [[nodiscard]] PrecisionTimerState state(const PrecisionTimerNode &node) noexcept;
        [[nodiscard]] size_t size() noexcept;

    private:
        [[nodiscard]] hal::CpuClockDeadline root_deadline_locked() const noexcept;
        void post_elapsed(PrecisionTimerNode &node) noexcept;

        tay::spinlock lock_{};
        timer_heap heap_{timer_heap::locate_type{}, TIMER_HEAP_CMP};
        DeadlineState *deadlines_ = nullptr;
    };

    [[nodiscard]] PrecisionTimerEngine &bsp_timer_engine() noexcept;
}  // namespace kernel::timer

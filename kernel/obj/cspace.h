 ...
 * @brief 扁平 CSpace、CapToken lookup 和基础 capability 操作。
 * @version 0.1.0-dev.1
 * @date 2026-08-12
 *
...
namespace cap {
     ...
     *
     * 句柄持有 `CapPin`，因此 `object()` 的借用指针在句柄释放前保持有效。rights 和 badge
     * 是 resolve 成功时的快照，不会随原 Capability slot 的后续删除而变化。
     *
     * @tparam T 声明静态 `TYPE` 的具体 KernelObject 类型。
     */
    template <CapabilityObject T>
    class CapObject final {
    public:
        /** @brief 接管已经完成类型检查的对象 pin。 */
        CapObject(CapPin &&pin) noexcept : pin_(std::move(pin)) {}
        CapObject(const CapObject &)                = delete;
        CapObject &operator=(const CapObject &)     = delete;
        CapObject(CapObject &&) noexcept            = default;
        CapObject &operator=(CapObject &&) noexcept = default;
     ...
     * 的 capability 强引用。
     */
    class CSpace final : public TypedKernelObject<CSpace, ObjectType::CSPACE> {
    public:
         ...
         */
        [[nodiscard]] static tay::expected<CSpace *, CapError> create() noexcept;
         ...
         */
        [[nodiscard]] tay::expected<u8_t, CapError> attach(CNode &node) noexcept;
         ...
         */
        [[nodiscard]] tay::expected<CNode *, CapError> detach(u8_t index) noexcept;
         ...
         * @param requested_slot 零表示自动分配，否则请求指定 slot。
         * @return 可解析 token，或目录、slot、generation 和对象状态错误。
         */
        [[nodiscard]] tay::expected<CapToken, CapError> install(KernelObject &object, u64_t rights,
                                                                u64_t badge      = 0,
         ...
         */
        [[nodiscard]] tay::expected<CapPin, CapError> resolve(
            CapToken token, ObjectType expected_type = ObjectType::NONE,
        ...
        /**
         * @brief 派生保留 source badge 的 Capability 副本。
         * @param source 具备 `RIGHT_COPY` 的源 token。
         * @param requested_rights 零表示保留源权限，否则必须是源权限子集。
         * @param destination_cnode 目标 CNode index。
         * @param requested_slot 零表示自动分配，否则请求指定 slot。
         * @return 新 token；失败时 source 和目标 free list 保持有效。
         */
        [[nodiscard]] tay::expected<CapToken, CapError> copy(
            CapToken source, u64_t requested_rights = 0, u8_t destination_cnode = ROOT_CNODE_INDEX,
            u16_t requested_slot = 0) noexcept;

        /**
         * @brief 派生具有权限子集和新 badge 的 Capability。
         * @param source 具备 `RIGHT_MINT` 的源 token。
         * @param requested_rights 新 Capability 的权限，必须是源权限子集。
         * @param badge 新 badge。
         * @param destination_cnode 目标 CNode index。
         * @param requested_slot 零表示自动分配，否则请求指定 slot。
         * @return 新 token；请求扩权或源 token 无效时返回错误。
         */
        [[nodiscard]] tay::expected<CapToken, CapError> mint(
            CapToken source, u64_t requested_rights, u64_t badge = 0,
            u8_t destination_cnode = ROOT_CNODE_INDEX, u16_t requested_slot = 0) noexcept;

        /**
         * @brief 删除单个 Capability，并把直接派生项提升到其父节点。
         * @note source slot 先失效，随后在节点锁外释放对象强引用。
         */
        [[nodiscard]] tay::expected<void, CapError> delete_cap(CapToken token) noexcept;

        /**
         * @brief 后序删除 source 的全部 CDT 后代，但保留 source 自身。
         * @param token 具备 `RIGHT_REVOKE` 的 source token。
         * @return 撤销成功或 token、权限、树一致性错误。
         */
        [[nodiscard]] tay::expected<void, CapError> revoke(CapToken token) noexcept;

    private:
        explicit CSpace(u16_t cookie) noexcept;

        /** @pre 调用方持有 `mutation_lock_`。 */
        [[nodiscard]] tay::expected<u32_t, CapError> next_generation_locked(
         ...
         */
        [[nodiscard]] bool locate_capability(Capability *capability, CNode *&node,
                                             u16_t &slot) noexcept;

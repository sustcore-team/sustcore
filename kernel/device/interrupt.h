...
namespace device::interrupt {
    ...
    public:
        ...
        [[nodiscard]] virtual FwId ctrl() const noexcept                             = 0;
        ...
        [[nodiscard]] virtual tay::expected<void, tay::error_code> complete(
            const IrqClaim &claim) noexcept                                                    = 0;
        [[nodiscard]] virtual tay::expected<void, tay::error_code> mask(u32_t hw_irq) noexcept = 0;
        [[nodiscard]] virtual tay::expected<void, tay::error_code> unmask(
            u32_t hw_irq) noexcept = 0;
        [[nodiscard]] virtual tay::expected<void, tay::error_code> set_priority(
            u32_t hw_irq, u32_t priority) noexcept = 0;
    };
    ...
    struct IrqCascadeFrame {
        IrqClaim parent{};
        IrqDomain *child_domain = nullptr;
    };

    enum class IrqSource : u8_t { TIMER, SOFTWARE, EXTERNAL };

    struct TrapLine {
        IrqSource source;
        xlen_t code;

        [[nodiscard]] friend constexpr bool operator==(TrapLine left,
                                                       TrapLine right) noexcept = default;
    };

    /** @brief 与架构 timer cause 编号无关的当前 CPU timer line。 */
    inline constexpr TrapLine TIMER_LINE{.source = IrqSource::TIMER, .code = 0};
    ...
    using Handler    = void (*)(void *handler_ctx, const TrapEvent &event) noexcept;
    using IrqHandler = void (*)(void *handler_ctx, const IrqLine &line) noexcept;
    ...
    [[nodiscard]] tay::expected<IrqLine, tay::error_code> resolve(FwId ctrl, u32_t hw_irq) noexcept;
     ...
     */
    [[nodiscard]] tay::expected<TrapSub, tay::error_code> subscribe(
        TrapLine line, Handler handler, void *handler_ctx = nullptr) noexcept;

    /**
     * @brief 取消架构 trap 源订阅，并同步等待既有 handler 离开。
     * @pre 不得在硬中断、IRQ handler 或仍持有 Registry RCU read guard 的路径调用。
     * @post 成功返回后 handler_ctx 可以立即销毁。
     */
    [[nodiscard]] tay::expected<void, tay::error_code> unsubscribe(TrapSub subscription) noexcept;
     ...
     * @pre 不得在硬中断、IRQ handler 或仍持有 Registry RCU read guard 的路径调用。
     */
    [[nodiscard]] tay::expected<IrqSub, tay::error_code> subscribe_irq(
        IrqLine line, IrqHandler handler, void *handler_ctx = nullptr) noexcept;
    /**

...
static bool pmm_add_free_memblock(void *paddr, int order) {
    ...
    if (((umb_t)paddr % block_size) != 0) {
        log_error("pmm_add_free_memblock: 地址 %p 非 %lu 对齐",
                  paddr,
                  block_size);
        return false;
    }

    log_debug ("添加了内存块 [%p, %p), 阶数=%d",
              paddr,
              (void *)((umb_t)paddr + block_size),
              order);
...
static bool pmm_add_free_pages(void *paddr, int pages) {
    ...
    while (remain > 0) {
        ...
        if (!pmm_add_free_memblock((void *)addr, order)) {
            log_error("pmm_add_free_pages: 添加内存块失败 addr=%p order=%d", (void *)addr, order);
            return false;
        }
        umb_t block_pages = 1ul << order;
...
static void *pmm_fetch_free_memblock(int order) {
    ...
    if (free_area_head[order] == nullptr) {
        ...
        log_debug("将[%p, %p)拆分为[%p, %p)和[%p, %p)",
                 higher_block,
                 (void *)((umb_t)higher_block + block_size * 2),
                 buddy1, (void *)((umb_t)buddy1 + block_size),
                 buddy2, (void *)((umb_t)buddy2 + block_size));
    ...
    log_debug ("分配了内存块 [%p, %p), 阶数=%d",
              paddr,
              (void *)((umb_t)paddr + (1ul << (order + 12))),
              order);
...
void pmm_testbench(void) {
    ...
    log_info("blk1=%p, blk2=%p, blk3=%p, blk4=%p, blk5=%p", blk1, blk2, blk3, blk4, blk5);
...
void pmm_init(PhyscialMemoryLayout *layout) {
    ...
    // STEP3: 利用 __HEAP__ 实现__primitive_kmalloc__函数, 设置为初始内存分配函数

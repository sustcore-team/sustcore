/**
 * @file entry.cpp
 * @author theflysong (song_of_the_fly@163.com)
 * @brief SBI RISC-V 早期分页入口
 * @version 0.1.0-dev.1
 * @date 2026-06-02
 *
 * @copyright Copyright (c) 2026
 *
 */

#include <boot/sbi/arch/riscv64/early_paging.h>
#include <sustcore/addr.h>
#include <tay/attribute.h>

#include <cstddef>

#define SBI_BOOT_STRING(x)  SBI_BOOT_RODATA constexpr const char x[]
#define SBI_BOOT_STRLEN(x)  (sizeof(x) - 1)
#define SBI_BOOT_WRITE(str) _sbi_write(SBI_BOOT_STRLEN(str), str)

namespace sbi {
    SBI_BOOT_STRING(SBI_BOOT_MSG)  = "SBI引导程序启动!\n";
    SBI_BOOT_STRING(SBI_PANIC_MSG) = "SBI引导程序发生错误, 无法继续执行!\n";
    SBI_BOOT_STRING(SBI_INVALID_KERNEL_SIZE_MSG)   = "错误: 内核大小超过 32MB  限制\n";
    SBI_BOOT_STRING(SBI_INVALID_PAGING_SIZE_MSG)   = "错误: 分页部分小于 64KB 限制\n";
    SBI_BOOT_STRING(SBI_MISALIGNED_PAGING_MSG)     = "错误: 分页部分地址未对齐到4KB\n";
    SBI_BOOT_STRING(SBI_BOUNDARY_MISALIGNED_MSG)   = "错误: 内核末尾地址未对齐到2MB\n";
    SBI_BOOT_STRING(SBI_1G_MISALIGNED_MSG)         = "错误: 1GB 映射地址未对齐\n";
    SBI_BOOT_STRING(SBI_PAGE_ALLOC_OVERFLOW_MSG)   = "错误: SBI 分页保留区耗尽\n";
    SBI_BOOT_STRING(SBI_CHECK_PASS_MSG)            = "SBI引导程序检查通过!\n";
    SBI_BOOT_STRING(SBI_PAGING_SETUP_COMPLETE_MSG) = "SBI分页设置完成!\n";
    SBI_BOOT_STRING(SBI_INVALID_DTB_MAGIC_MSG)     = "错误: DTB魔数不正确\n";
    SBI_BOOT_STRING(SBI_INVALID_DTB_SIZE_MSG)      = "错误: DTB大小超过限制\n";
    SBI_BOOT_STRING(SBI_MISALIGNED_KPA_MSG)        = "错误: KPA区域映射失效!\n";
}  // namespace sbi

namespace sbi {
    extern "C" size_t __sbi_boot_hart_id;
    extern "C" addr_t __sbi_dtb_phys;
    extern "C" addr_t __sbi_reclaimable_cursor;
    static_assert(sizeof(__sbi_boot_hart_id) == 8, "类型大小不匹配");
    static_assert(sizeof(__sbi_dtb_phys) == 8, "类型大小不匹配");

    extern "C" SBI_BOOT_TEXT void _sbi_write(size_t len, const char *buf);

    SBI_BOOT_TEXT void _sbi_panic() {
        SBI_BOOT_WRITE(SBI_PANIC_MSG);
        while (true);
    }

#define SBI_BOOT_PANIC(x)  \
    do {                   \
        SBI_BOOT_WRITE(x); \
        _sbi_panic();      \
    } while (0)

    static SBI_BOOT_BSS addr_t reclaimable_cursor;
    static SBI_BOOT_BSS addr_t reclaimable_limit;
    static SBI_BOOT_BSS addr_t root_page_table;
    static SBI_BOOT_BSS addr_t kernel_kva_limit;

    // 将初始 8000'0000 ~ C000'0000 区域 (1GB) 映射到 PA 与 KPA 中
    static constexpr addr_t PA_START = 0x80000000;
    static constexpr addr_t PA_LIMIT = 0xC0000000;

    SBI_BOOT_TEXT void page_zero(addr_t pa) {
        auto *page = reinterpret_cast<byte *>(pa);
        for (size_t i = 0; i < PAGE_SIZE; ++i) {
            page[i] = 0;
        }
    }

    SBI_BOOT_TEXT addr_t page_alloc() {
        addr_t current = reclaimable_cursor;
        if ((current & (PAGE_TABLE_ALIGNMENT - 1)) != 0) {
            SBI_BOOT_PANIC(SBI_MISALIGNED_PAGING_MSG);
        }
        if (current + PAGE_SIZE > reclaimable_limit) {
            SBI_BOOT_PANIC(SBI_PAGE_ALLOC_OVERFLOW_MSG);
        }
        reclaimable_cursor = current + PAGE_SIZE;
        page_zero(current);
        return current;
    }

    SBI_BOOT_TEXT PteType *page_table(addr_t pa) {
        // 开启分页前物理地址可直接访问，因此可将页帧地址解释为页表数组。
        return reinterpret_cast<PteType *>(pa);
    }

    SBI_BOOT_TEXT PteType *ensure_next_level(PteType *table, size_t index) {
        PteType &entry = table[index];
        if ((entry & PDE_BASE) == 0) {
            addr_t next_level = page_alloc();
            entry             = MAKE_PDE(next_level);
        }
        return page_table((entry & PPN_MASK) >> PPN_SHIFT << 12);
    }

    SBI_BOOT_TEXT void mapping_in_2m(addr_t root, addr_t va, addr_t pa) {
        if ((va & PAGING_ALIGNMENT_MASK) != 0 || (pa & PAGING_ALIGNMENT_MASK) != 0) {
            SBI_BOOT_PANIC(SBI_BOUNDARY_MISALIGNED_MSG);
        }

        size_t vpn[3];
        TOVPN(vpn, va);

        auto *l0   = page_table(root);
        auto *l1   = ensure_next_level(l0, vpn[2]);
        l1[vpn[1]] = MAKE_PTE(pa);
    }

    SBI_BOOT_TEXT void mapping_in_1g(addr_t root, addr_t va, addr_t pa) {
        if ((va & PAGE_SIZE_1G_MASK) != 0 || (pa & PAGE_SIZE_1G_MASK) != 0) {
            SBI_BOOT_PANIC(SBI_1G_MISALIGNED_MSG);
        }

        size_t vpn[3];
        TOVPN(vpn, va);

        auto *l0   = page_table(root);
        l0[vpn[2]] = MAKE_PTE(pa);
    }

    SBI_BOOT_TEXT void map_range_in_2m(addr_t root, addr_t va_s, addr_t va_e, addr_t pa_s) {
        addr_t va = va_s;
        addr_t pa = pa_s;
        while (va < va_e) {
            mapping_in_2m(root, va, pa);
            va += PAGE_SIZE_2M;
            pa += PAGE_SIZE_2M;
        }
    }

    SBI_BOOT_TEXT void map_identity_and_kpa(addr_t root, addr_t pa_s, addr_t pa_e) {
        addr_t current = pa_s;

        if ((pa_s & PAGE_SIZE_1G_MASK) != 0 || (pa_e & PAGE_SIZE_1G_MASK) != 0) {
            SBI_BOOT_PANIC(SBI_MISALIGNED_KPA_MSG);
        }

        while (current + PAGE_SIZE_1G <= pa_e) {
            mapping_in_1g(root, current, current);
            mapping_in_1g(root, current + KPA_START, current);
            current += PAGE_SIZE_1G;
        }
    }

    SBI_BOOT_TEXT size_t calc_satp() {
        return SATP_SV39_BASE | TO_PPN(root_page_table);
    }

    SBI_BOOT_TEXT qword dtb_byte(size_t off) {
        return *(byte *)(__sbi_dtb_phys + off);
    }

#define DTB_BYTE(x)  dtb_byte(x)
#define DTB_WORD(x)  (DTB_BYTE(x) << 8 | DTB_BYTE((x) + 1))
#define DTB_DWORD(x) (DTB_WORD(x) << 16 | DTB_WORD((x) + 2))

    extern "C" SBI_BOOT_TEXT size_t _sbi_setup() {
        SBI_BOOT_WRITE(SBI_BOOT_MSG);

        char *paging_start = &s_sbi_reclaimable;
        char *paging_end   = &e_sbi_reclaimable;

        size_t paging_sz = paging_end - paging_start;
        if (paging_sz < MINIMUM_PAGING_SIZE) {
            SBI_BOOT_PANIC(SBI_INVALID_PAGING_SIZE_MSG);
        }
        if ((reinterpret_cast<addr_t>(paging_start) & (PAGE_SIZE - 1)) != 0) {
            SBI_BOOT_PANIC(SBI_MISALIGNED_PAGING_MSG);
        }

        // 阶段一：以链接脚本预留区作为无堆页表分配器。
        reclaimable_cursor = reinterpret_cast<addr_t>(paging_start);
        reclaimable_limit  = reinterpret_cast<addr_t>(paging_end);

        char *kernel_start = &s_sbi;
        char *kernel_end   = &ekernel_phys;
        size_t kernel_sz   = kernel_end - kernel_start;

        if (kernel_sz > MAXIMUM_KERNEL_SIZE) {
            SBI_BOOT_PANIC(SBI_INVALID_KERNEL_SIZE_MSG);
        }

        auto kernel_end_arith = reinterpret_cast<addr_t>(kernel_end);
        if ((kernel_end_arith & PAGING_ALIGNMENT_MASK) != 0) {
            SBI_BOOT_PANIC(SBI_BOUNDARY_MISALIGNED_MSG);
        }

        auto kernel_start_arith   = reinterpret_cast<addr_t>(kernel_start);
        auto aligned_kernel_start = kernel_start_arith & ~PAGING_ALIGNMENT_MASK;

        SBI_BOOT_WRITE(SBI_CHECK_PASS_MSG);

        dword magic = DTB_DWORD(0);
        if (magic != 0xD00DFEED) {
            SBI_BOOT_PANIC(SBI_INVALID_DTB_MAGIC_MSG);
        }
        dword dtb_sz = DTB_DWORD(4);
        if (dtb_sz > MAXIMUM_DTB_SIZE) {
            SBI_BOOT_PANIC(SBI_INVALID_DTB_SIZE_MSG);
        }

        root_page_table  = page_alloc();
        kernel_kva_limit = kernel_end_arith + KVA_START;

        // 阶段二：恒等映射维持切换瞬间的执行流，KPA/高半区映射服务后续内核。
        map_identity_and_kpa(root_page_table, PA_START, PA_LIMIT);
        map_range_in_2m(root_page_table, aligned_kernel_start + KVA_START, kernel_kva_limit,
                        aligned_kernel_start);

        auto aligned_dtb_start = __sbi_dtb_phys & ~PAGING_ALIGNMENT_MASK;
        auto dtb_end           = __sbi_dtb_phys + dtb_sz;
        auto aligned_dtb_end   = (dtb_end + PAGING_ALIGNMENT_MASK) & ~PAGING_ALIGNMENT_MASK;
        map_range_in_2m(root_page_table, aligned_dtb_start + KVA_START, aligned_dtb_end + KVA_START,
                        aligned_dtb_start);

        // 阶段三：把已消耗的临时区边界交给高半区入口，供 BootInfo 标记回收范围。
        __sbi_reclaimable_cursor = reclaimable_cursor;
        SBI_BOOT_WRITE(SBI_PAGING_SETUP_COMPLETE_MSG);
        return calc_satp();
    }
}  // namespace sbi

...
namespace sbi {
    ...
    extern "C" _SBI_TEXT void _sbi_write(size_t len, const char *buf);

    _SBI_TEXT void _sbi_panic() {
...
#define _SBI_PANIC(x)  \
    ...
    static _SBI_BSS addr_t reclaimable_cursor;
    static _SBI_BSS addr_t reclaimable_limit;
    static _SBI_BSS addr_t root_page_table;
    static _SBI_BSS addr_t kernel_kva_limit;
    ...
    _SBI_TEXT void page_zero(addr_t pa) {
    ...
    _SBI_TEXT PteType *page_table(addr_t pa) {
        // 开启分页前物理地址可直接访问，因此可将页帧地址解释为页表数组。
        return reinterpret_cast<PteType *>(pa);
    }

    _SBI_TEXT PteType *ensure_next_level(PteType *table, size_t index) {
    ...
    _SBI_TEXT void mapping_in_2m(addr_t root, addr_t va, addr_t pa) {
    ...
    _SBI_TEXT void mapping_in_1g(addr_t root, addr_t va, addr_t pa) {
    ...
    _SBI_TEXT void map_range_in_2m(addr_t root, addr_t va_s, addr_t va_e, addr_t pa_s) {
        addr_t va = va_s;
        addr_t pa = pa_s;
        while (va < va_e) {
    ...
    _SBI_TEXT void map_identity_and_kpa(addr_t root, addr_t pa_s, addr_t pa_e) {
        addr_t current = pa_s;
    ...
    _SBI_TEXT size_t calc_satp() {
        return SATP_SV39_BASE | TO_PPN(root_page_table);
    }

    _SBI_TEXT qword dtb_byte(size_t off) {
...
#define DTB_DWORD(x) (DTB_WORD(x) << 16 | DTB_WORD((x) + 2))
    ...
    extern "C" _SBI_TEXT size_t _sbi_setup() {

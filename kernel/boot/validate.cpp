...
namespace boot::early_internal {
    namespace detail::validation {
        ...
        [[nodiscard]] BOOT_INIT_TEXT size_t bootinfo_prefix_sz(size_t region_cnt) noexcept {
            constexpr size_t FIXED_SIZE = sizeof(BootInfoHeader) + sizeof(PhyAddr);
            constexpr size_t MAX_REGION_COUNT =
                (MAX_BOOTINFO_SIZE - FIXED_SIZE) / sizeof(MemoryRegion);
            if (region_cnt == 0 || region_cnt > MAX_REGION_COUNT ||
                region_cnt > MAX_BOOTINFO_REGIONS)
            {
        ...
        BOOT_INIT_TEXT void validate_regions(const BootInfoHeader *header) noexcept {
            ...
            for (size_t idx = 0; idx < header->region_cnt; ++idx) {
                ...
                if (!valid_memory_type(region.type) || begin >= end ||
                    !region.area.begin.aligned<PAGE_SIZE>() ||
            ...
            for (size_t parent_idx = 0; parent_idx < parent_cnt; ++parent_idx) {
                ...
                while (true) {
                    if (child_idx < parent_cnt || child_idx >= header->region_cnt ||
                        visited[child_idx])
        ...
        BOOT_INIT_TEXT void validate_fdt(const BootInfoHeader *header) noexcept {
            ...
            for (size_t idx = 0; idx < header->region_cnt; ++idx) {
                if (regions[idx].area.begin.arith() <= dtb_paddr &&
                    dtb_end <= regions[idx].area.end.arith() &&
                    regions[idx].type != MemoryType::FREE)
                {
    ...
    BOOT_INIT_TEXT void validate_bootinfo(size_t bsp_hwid, const BootInfoHeader *header) noexcept {
        ...
        const size_t prefix_sz = detail::validation::bootinfo_prefix_sz(header->region_cnt);
        if (header->info_sz < prefix_sz) {
            kernel::log::panic("BootInfo 被截断");
        }

        detail::validation::validate_regions(header);
        detail::validation::validate_fdt(header);
    }
}  // namespace boot::early_internal

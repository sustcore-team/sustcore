...
namespace kernel::test::cases {
    namespace {
        ...
        public:
            ...
            void run() noexcept {
                ...
                if (running_.load(std::memory_order_acquire) || completion_.pending() ||
                    engine.state(timer_) != kernel::timer::PrecisionTimerState::IDLE)
                    kernel::test::fail("重复启动 periodic timer probe");

                dispatch_count_ = 0;
                running_.store(true, std::memory_order_release);
                next_deadline_ = kernel::timer::saturated_deadline_after(
        ...
        private:
            void complete() noexcept {
                ...
                const auto now = hal::CpuClock::instance().current_time();
                next_deadline_ = kernel::timer::saturated_deadline_after(now, PERIOD);
                arm_next();
            }

            void arm_next() noexcept {
                auto armed =
                    kernel::timer::bsp_timer_engine().arm(timer_, next_deadline_, completion_);
                if (!armed)
        ...
        void timed_wait_entry(void *opaque) noexcept {
            ...
            auto result = scheduler::instance().current_thread().wait_until(state->deadline);
            if (!result)
        ...
        void repeated_timed_wait_entry(void *opaque) noexcept {
            auto *state = static_cast<RepeatedTimedWaitCase *>(opaque);
            if (state == nullptr)
                kernel::test::fail("repeated timed-wait 用例没有状态");
            auto &current = scheduler::instance().current_thread();
        ...
        void timer_deadline_competitor_entry(void *) noexcept {}

        [[nodiscard]] cap::ObjectRef<task::Thread> start_timed_wait_case(
            TimedWaitCase &state) noexcept {
            auto thread =
                task::Thread::create_kernel(task::kernel_process(), timed_wait_entry, &state);
            if (!thread)
                kernel::test::fail("无法创建 timed-wait Thread");
            if (auto attached = scheduler::instance().attach(**thread); !attached)
            ...
            if ((*thread)->state() == task::ThreadState::BLOCKED &&
                (*thread)->timed_wait_generation() == 0)
                kernel::test::fail("已阻塞 timed-wait Thread 没有 generation");
            return std::move(*thread);
        }

        void wait_for_timed_wait_exit(task::Thread &thread) noexcept {
    ...
    void run_precision_timer(Context &) noexcept {
        auto &clock                        = hal::CpuClock::instance();
        auto &engine                       = kernel::timer::bsp_timer_engine();
        auto &deadlines                    = kernel::timer::bsp_deadline_state();
        const bool interrupts_were_enabled = hal::interrupts_enabled();
        if (!interrupts_were_enabled)
            hal::enable_interrupts();

        TimedWaitCase timeout_case{
            .deadline = kernel::timer::saturated_deadline_after(clock.current_time(), 2_ms),
        };
        auto timeout_thread = start_timed_wait_case(timeout_case);
        wait_for_timed_wait_exit(*timeout_thread);
        kernel::test::require(
            timeout_case.completed && timeout_case.result == task::TimedWaitResult::TIMEOUT,
            "timed wait 未按 timeout 完成");
        timeout_thread.reset();

        const auto now = clock.current_time();
        TimedWaitCase past_case{
            .deadline = now == units::time{} ? now : now - 1_ns,
        };
        auto past_thread = start_timed_wait_case(past_case);
        wait_for_timed_wait_exit(*past_thread);
        kernel::test::require(past_case.result == task::TimedWaitResult::TIMEOUT,
                              "过期 timed wait 未异步超时");
        past_thread.reset();

        TimedWaitCase wake_case{
            .deadline = kernel::timer::saturated_deadline_after(clock.current_time(), 1_s),
        };
        auto wake_thread            = start_timed_wait_case(wake_case);
        const u64_t wake_generation = wake_thread->timed_wait_generation();
        kernel::test::require(
            !scheduler::instance().detach(*wake_thread, scheduler::DetachReason::SUSPEND) &&
                !scheduler::instance().detach(*wake_thread, scheduler::DetachReason::TERMINATE),
            "scheduler detach 了活跃 timed wait 中的 Thread");

        TimedWaitCase nonroot_case{
            .deadline = kernel::timer::saturated_deadline_after(clock.current_time(), 2_s),
        };
        auto nonroot_thread            = start_timed_wait_case(nonroot_case);
        const u64_t nonroot_generation = nonroot_thread->timed_wait_generation();
        kernel::test::require(nonroot_thread->cancel_timed_wait(nonroot_generation),
                              "non-root timed-wait cancel 丢失");
        wait_for_timed_wait_exit(*nonroot_thread);
        ...
        auto competitor =
            task::Thread::create_kernel(task::kernel_process(), timer_deadline_competitor_entry);
        if (!competitor || !scheduler::instance().attach(**competitor))
            kernel::test::fail("无法创建 timer/RR deadline 竞争者");
        const auto timer_source      = deadlines.timer_deadline();
        const auto preemption_source = deadlines.preemption_deadline();
        const auto programmed        = deadlines.programmed_deadline();
        ...
        kernel::test::require(!wake_thread->wake_timed_wait(wake_generation - 1) &&
                                  wake_thread->wake_timed_wait(wake_generation) &&
                                  !wake_thread->wake_timed_wait(wake_generation),
                              "timed-wait wake 未准确选择一个胜者");
        wait_for_timed_wait_exit(*wake_thread);
        kernel::test::require(wake_case.result == task::TimedWaitResult::WOKEN,
                              "timed-wait wake 返回了错误结果");
        wake_thread.reset();

        RepeatedTimedWaitCase repeated{
            .first_deadline  = kernel::timer::saturated_deadline_after(clock.current_time(), 1_s),
            .second_deadline = kernel::timer::saturated_deadline_after(clock.current_time(), 1_s),
        };
        auto repeated_thread = task::Thread::create_kernel(task::kernel_process(),
                                                           repeated_timed_wait_entry, &repeated);
        if (!repeated_thread || !scheduler::instance().attach(**repeated_thread))
            kernel::test::fail("无法创建 repeated timed-wait Thread");
        scheduler::yield();
        const u64_t first_generation = (*repeated_thread)->timed_wait_generation();
        kernel::test::require((*repeated_thread)->wake_timed_wait(first_generation),
                              "repeated timed-wait 首次 wake 丢失");
        while (!repeated.first_completed ||
               (*repeated_thread)->timed_wait_generation() == first_generation)
            scheduler::yield();
        const u64_t second_generation = (*repeated_thread)->timed_wait_generation();
        kernel::test::require(!(*repeated_thread)->cancel_timed_wait(first_generation) &&
                                  (*repeated_thread)->cancel_timed_wait(second_generation) &&
                                  !(*repeated_thread)->cancel_timed_wait(second_generation),
                              "timed-wait stale/duplicate cancel 语义错误");
        wait_for_timed_wait_exit(**repeated_thread);
        ...
        const auto shared_deadline =
            kernel::timer::saturated_deadline_after(clock.current_time(), 2_ms);
        TimedWaitCase equal_cases[2] = {
            TimedWaitCase{.deadline = shared_deadline},
            TimedWaitCase{.deadline = shared_deadline},
        };
        auto equal_first  = start_timed_wait_case(equal_cases[0]);
        auto equal_second = start_timed_wait_case(equal_cases[1]);
        wait_for_timed_wait_exit(*equal_first);
        wait_for_timed_wait_exit(*equal_second);
    ...
    void run_periodic_timer_probe(Context &) noexcept {
        const bool interrupts_were_enabled = hal::interrupts_enabled();
        if (!interrupts_were_enabled)
            hal::enable_interrupts();

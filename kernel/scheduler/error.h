/**
 * @file error.h
 * @brief 定义 SchedulerCore 可恢复的生命周期与状态转换错误。
 */
...
namespace scheduler {
    ...
    public:
        ...
        struct DeadlineSinkAlreadyInstalled {};
        ...
        [[nodiscard]] kernel::KernelError code() const noexcept {
            ...
            return visit(tay::overloaded{
                ...
                [](const DeadlineSinkAlreadyInstalled &) noexcept {
                    return Reason::DEADLINE_SINK_ALREADY_INSTALLED;
                },
    ...
    private:
        using Storage =
            tay::variant<NotReady, AlreadyReady, BootstrapNotRunning, InterruptsEnabled,
                         ...
                         DeadlineSinkAlreadyInstalled, InvalidDeadlineSink>;

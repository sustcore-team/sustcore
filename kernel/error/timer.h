/**
 * @file timer.h
 * @brief 定义 HRTQueue::arm 的可恢复状态错误。
 */
...
namespace kernel::timer {
    ...
    public:
        ...
        struct NodeNotIdle {
            HRTState state = HRTState::IDLE;
        };
        struct NodeAlreadyLinked {};
        struct CompletionBusy {};
        ...
        [[nodiscard]] kernel::KernelError code() const noexcept {
            ...
            return visit(tay::overloaded{
                ...
                [](const CompletionBusy &) noexcept { return Reason::COMPLETION_BUSY; },
            });
        }
    ...
    private:
        using Storage = tay::variant<WorkQueueNotAccepting, EngineNotInitialized, NodeNotIdle,
                                     NodeAlreadyLinked, CompletionBusy>;

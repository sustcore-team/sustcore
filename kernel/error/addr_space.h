...
namespace task {
    ...
    public:
        ...
        AddrSpaceError()                                      = delete;
        AddrSpaceError(const AddrSpaceError &)                = default;
        AddrSpaceError &operator=(const AddrSpaceError &)     = default;
        AddrSpaceError(AddrSpaceError &&) noexcept            = default;
        AddrSpaceError &operator=(AddrSpaceError &&) noexcept = default;
        ~AddrSpaceError() noexcept                            = default;
        ...
        [[nodiscard]] kernel::KernelError code() const noexcept {
            using Reason = kernel::KernelError::AddrSpaceError;
            return visit(tay::overloaded{
                ...
                [](const SegOffsetOutOfRange &) noexcept {
                    return Reason::SEG_OFFSET_OUT_OF_RANGE;
                },
    ...
    private:
        using Storage = tay::variant<InvalidSegment, InvalidArea, InvalidFlags, SegOffsetOutOfRange,
                                     MappingExceedsSegment, AccessDenied, VmaOverlap, VmaNotOwned,
                                     UnmappedAddress, MappingChanged, BackingAllocationFailed,
                                     PageTableFailed, OutOfMemory>;

    public:
        template <typename Alternative>
            requires std::is_constructible_v<Storage, Alternative>
        AddrSpaceError(Alternative alternative) noexcept : value_(std::move(alternative)) {}

    private:
        Storage value_;
    };

    static_assert(sizeof(AddrSpaceError) <= 48);
    static_assert(std::is_nothrow_move_constructible_v<AddrSpaceError>);
}  // namespace task

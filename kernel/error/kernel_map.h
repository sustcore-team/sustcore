...
namespace memory {
    ...
    public:
        ...
        KernelMapError()                                      = delete;
        KernelMapError(const KernelMapError &)                = default;
        KernelMapError &operator=(const KernelMapError &)     = default;
        KernelMapError(KernelMapError &&) noexcept            = default;
        KernelMapError &operator=(KernelMapError &&) noexcept = default;
        ~KernelMapError() noexcept                            = default;
        ...
        [[nodiscard]] kernel::KernelError code() const noexcept {
            using Reason = kernel::KernelError::KernelMapError;
            return visit(tay::overloaded{
                [](const InitAlreadyAttempted &) noexcept {
                    return Reason::INIT_ALREADY_ATTEMPTED;
                },
    ...
    private:
        using Storage =
            tay::variant<InitAlreadyAttempted, DependencyNotReady, InvalidKernelLayout,
    ...
    public:
        ...
        KernelMapError(Alternative alternative) noexcept : value_(std::move(alternative)) {}

    private:
        Storage value_;
    };

    static_assert(sizeof(KernelMapError) <= 56);
    static_assert(std::is_nothrow_move_constructible_v<KernelMapError>);
}  // namespace memory

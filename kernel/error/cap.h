...
namespace cap {
    ...
    public:
        ...
        CError(CError &&) noexcept            = default;
        CError &operator=(CError &&) noexcept = default;
        ~CError() noexcept                    = default;
        ...
        [[nodiscard]] kernel::KernelError code() const noexcept {
            using Reason = kernel::KernelError::CError;
    ...
    public:
        ...
        CError(Alternative alternative) noexcept : value_(std::move(alternative)) {}

    private:
        Storage value_;
    };

    static_assert(sizeof(CError) <= 32);
    static_assert(std::is_nothrow_move_constructible_v<CError>);
}  // namespace cap

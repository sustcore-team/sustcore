...
namespace memory::paging {
    namespace {
        ...
        [[nodiscard]] PageDesc &retire_desc(PhyAddr page, PtOwnerId owner) noexcept {
            if (!page.nonnull() || !page.aligned<PAGE_SIZE>())
                kernel::log::panic("无效的页表回收操作");
            auto *descriptor = page_db().lookup(page);
    ...
    tay::expected<PhyAddr, tay::error_code> PageAllocator::allocate(PtOwnerId owner) noexcept {
    ...
    void PageAllocator::retire(PhyAddr page, PtOwnerId owner) noexcept {
        if (!page.nonnull() || !page.aligned<PAGE_SIZE>())
            return;
        OwnedPages::resume(PageAlloc{.base = page, .pages = 1}, PageKind::PAGE_TABLE, owner)
            .release();
    }

    void RetirementSink::defer_table(PhyAddr page) noexcept {
        auto &descriptor = retire_desc(page, owner_);

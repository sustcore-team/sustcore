...
namespace memory {
    namespace {
        [[nodiscard]] bool domain_accepts(addr_t address, size_t bytes,
            ...
            if (domain == paging::WalkDomain::TRAMPOLINE_IDENTITY)
                return address < KPA_START && last < KPA_START;
            return address >= KPA_START && last >= KPA_START;
        }
    }  // namespace

    tay::expected<PhyAddr, PagingError> PageTable::create_root(PtOwnerId owner) noexcept {
        if (owner == 0)
        ...
        return paging::PageAllocator::allocate(owner).transform_error(
            [](tay::error_code error) noexcept -> PagingError {
                return PagingError::PtAllocFailed(static_cast<u8_t>(hal::PtOps::TOP_LEVEL),
                                                  kernel::from_tay_error(error).value_or(
                                                      kernel::KernelError::TayError::INTERNAL));
            });
    }

    void PageTable::destroy_root(PhyAddr root, PtOwnerId owner) noexcept {
        paging::PageAllocator::retire(root, owner);
    }

    PageTable::PageTable(PhyAddr root, PtKind kind, PtOwnerId owner, u16_t asid) noexcept {
        if (!adopt_root(root, kind, owner, asid))
            kernel::log::panic("无效的显式 PageTable 根节点");
    }

    tay::expected<void, PagingError> PageTable::adopt_root(PhyAddr root, PtKind kind,
                                                           PtOwnerId owner, u16_t asid) noexcept {
        const auto *descriptor = page_db().lookup(root);
    ...
    PageTable::~PageTable() noexcept {
        ...
        paging::detach_owned_tree(
            root_, owner_,
            kind_ == PtKind::KERNEL ? paging::WalkDomain::KERNEL_OWNED
                                    : paging::WalkDomain::USER_PRIVATE,
            retirements, [](const paging::Mapping &mapping) noexcept {
                if (auto *descriptor =
                        page_db().lookup(mapping.mapping.physical.align_down(PAGE_SIZE));
                    descriptor != nullptr)
        ...
        for (; mapped < pages; ++mapped) {
            ...
            if (!result) {
                while (mapped != 0) {
                    ...
                    if (auto *descriptor =
                            page_db().lookup(unmapped->mapping.physical.align_down(PAGE_SIZE));
                        descriptor != nullptr)
                        descriptor->map_count.fetch_sub(1, std::memory_order_relaxed);
                }
                shootdown(vaddr, pages);
                retirements.retire_all();
                return TAY_ERR(result);
            }
            if (auto *descriptor = page_db().lookup(physical + mapped * PAGE_SIZE);
                descriptor != nullptr)
    ...
    void PageTable::shootdown(addr_t vaddr, size_t pages) noexcept {
        smp::shootdown(
            RootBinding{.private_root = root_,
                        .asid         = asid_,
                        .role = kind_ == PtKind::KERNEL ? RootRole::KERNEL : RootRole::CLIENT},
            vaddr, pages);
    }
    ...
    PageTable::EntryType PageTable::root_entry(size_t index) const noexcept {
        if (index >= hal::PtOps::ENTRIES_PER_TABLE)
            return 0;
        return hal::PtOps::load_entry(&hal::PtOps::table(root_)[index]);
    }

    bool PageTable::try_install_root(size_t index, EntryType value) noexcept {
        if (index >= hal::PtOps::ENTRIES_PER_TABLE || !hal::PtOps::present(value))
            return false;
        auto *entry = &hal::PtOps::table(root_)[index];
        if (hal::PtOps::present(hal::PtOps::load_entry(entry)))
            return false;
        hal::PtOps::publish_table(entry, value);
        return true;
    }
}  // namespace memory

...
namespace memory::detail {
    PhyAddr symbol_paddr(const char *symbol) noexcept {
    ...
    PhyAddr kernel_start_paddr() noexcept {
        return symbol_paddr(skernel);
    }
    PhyAddr kernel_end_paddr() noexcept {
        return symbol_paddr(ekernel);
    }

    tay::expected<void, tay::error_code> map_kernel_layout(KernelVm &space) noexcept {
        const auto map_segment = [&space](char *begin, char *end, PageFlags flags) {
            if (begin == end)
                return tay::Ok();
            return space.map(reinterpret_cast<addr_t>(begin), symbol_paddr(begin),

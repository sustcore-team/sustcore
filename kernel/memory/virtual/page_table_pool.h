...
namespace memory {
    ...
    namespace paging {
        ...
        public:
             ...
             */
            static void retire(PhyAddr page, PageTableOwnerId owner) noexcept;
        };

        /**
         * @brief 范围事务使用的无分配页表页退役队列。
         *
         * 队列链接保存在 PageDescriptor 元数据中，而不触碰已经从页表树摘下的页表页内容；
         * 这样在 shootdown 完成前仍可安全保留硬件 page-table walker 所见的旧内容。
         */
        class RetirementSink final {
        public:
            explicit RetirementSink(PageTableOwnerId owner) noexcept : owner_(owner) {}

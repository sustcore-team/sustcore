/**
 * @file gfp.h
 * @brief Buddy 与 PageDatabase 之间的原子页所有权入口。
 */
...
namespace memory {
    /**
     * @brief 同时拥有 Buddy 分配和 PageDatabase claim 的物理页范围。
     *
    ...
    public:
        ...
        /** @brief 立即解除 PageDatabase 所有权并将完整范围归还 Buddy。 */
        void release() noexcept;

        /** @brief 恢复对已由 `detach()` 转移出去的页面范围的 RAII 所有权。 */
        [[nodiscard]] static OwnedPages resume(PageAllocation allocation, PageKind kind,
                                               u64_t owner_id) noexcept {
            return OwnedPages(std::move(allocation), kind, owner_id);
        }

        /**
         * @brief 将已 claim 页面的释放责任交给只保存 PageAllocation 的既有子系统。
         * @note 调用后本对象变为空；接收方必须最终通过 `resume()` 恢复 RAII 所有权。
         */
        [[nodiscard]] PageAllocation detach() noexcept;

    private:
        friend tay::expected<OwnedPages, tay::error_code> gfp(size_t, size_t, PageKind,
                                                              u64_t) noexcept;

        constexpr OwnedPages(PageAllocation allocation, PageKind kind, u64_t owner_id) noexcept
    ...
    /**
     * @brief 分配物理页并在同一 Buddy 临界区内完成 PageDatabase claim。
     */

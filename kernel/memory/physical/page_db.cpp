...
namespace memory {
    namespace {
        constinit PageDb database;

        [[nodiscard]] constexpr size_t pages_for_descs(size_t page_count) noexcept {
        ...
        [[nodiscard]] PageKind kind_for(MemoryType type) noexcept {
            ...
            kernel::log::panic("构造 PageDb 时遇到无效内存类型");
        }

        void initialize_range(PageDesc *base, size_t first, size_t count, PageKind kind,
    ...
    PageDb &page_db() noexcept {
        return database;
    }

    void PageDb::initialize(const BootInfoHeader &bootinfo) noexcept {
        if (initialized_)
            kernel::log::panic("PageDb 被重复初始化");
        ...
        if (region_count_ == 0 || region_count_ > MAX_BOOTINFO_REGIONS)
            kernel::log::panic("无效的 PageDb 父区域数量");
        ...
        for (size_t parent_idx = 0; parent_idx < region_count_; ++parent_idx) {
            ...
            const size_t metadata_pages = pages_for_descs(page_count);
            if (metadata_pages == 0 || metadata_pages > addr_t(-1) / PAGE_SIZE)
                kernel::log::panic("无效的 PageDb 元数据大小");
            ...
            auto *descriptors = reinterpret_cast<PageDesc *>(PA2KPA(backing.begin.arith()));
            for (size_t page = 0; page < page_count; ++page) new (&descriptors[page]) PageDesc{};
    ...
    PhysRegion *PageDb::find_region(PhyAddr address) noexcept {
        return const_cast<PhysRegion *>(static_cast<const PageDb &>(*this).find_region(address));
    }

    const PhysRegion *PageDb::find_region(PhyAddr address) const noexcept {
    ...
    PageDesc *PageDb::lookup(PhyAddr address) noexcept {
        return const_cast<PageDesc *>(static_cast<const PageDb &>(*this).lookup(address));
    }

    const PageDesc *PageDb::lookup(PhyAddr address) const noexcept {
    ...
    bool PageDb::claim(PhyArea area, PageKind kind, u64_t owner_id) noexcept {
    ...
    void PageDb::release(PhyArea area, PageKind kind, u64_t owner_id) noexcept {
        kernel::assert_task_ctx();
        for (addr_t current = area.begin.arith(); current < area.end.arith(); current += PAGE_SIZE)
        {
            auto *descriptor = lookup(PhyAddr(current));
            if (descriptor == nullptr || descriptor->state != PageState::CLAIMED ||
                ...
                kernel::log::panic("无效的 PageDb 释放操作");
        }
        for (addr_t current = area.begin.arith(); current < area.end.arith(); current += PAGE_SIZE)
        {
            auto *descriptor = lookup(PhyAddr(current));
    ...
    void PageDb::release_reclaim(PhyArea area) noexcept {
        kernel::assert_task_ctx();
        for (addr_t current = area.begin.arith(); current < area.end.arith(); current += PAGE_SIZE)
        {
            auto *descriptor = lookup(PhyAddr(current));
            if (descriptor == nullptr || descriptor->state != PageState::RESERVED ||
                ...
                kernel::log::panic("无效的可回收引导 PageDesc 状态转换");
        }
        for (addr_t current = area.begin.arith(); current < area.end.arith(); current += PAGE_SIZE)
        {
            auto *descriptor = lookup(PhyAddr(current));
    ...
    const PhysRegion &PageDb::region(size_t index) const noexcept {
        if (index >= region_count_)
            kernel::log::panic("PageDb 区域索引越界");
        return regions_[index];
    }
}  // namespace memory

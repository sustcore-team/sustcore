...
namespace memory {
     ...
     *
     * 除初始化早期外，调用者必须通过 buddy() 获取实例，以便由同一把 IRQ-safe
     * ticket spinlock 串行化所有 free-list 和 descriptor pool 操作。
     */
    class Buddy final {
    public:
        static constexpr size_t MAX_ORDER             = 30;
        static constexpr size_t DESCRIPTOR_POOL_PAGES = 4;
         ...
         */
        void put_range(PhyArea area) noexcept;
         ...
         */
        [[nodiscard]] tay::expected<PageAllocation, tay::error_code> try_gfp_in_order(
         ...
         */
        [[nodiscard]] tay::expected<PageAllocation, tay::error_code> try_get_free_pages(
            size_t pages, size_t alignment_pages = 1) noexcept;

        /**
         * @brief 将完整 PageAllocation 归还物理页分配器。
         * @param allocation 先前由同一 Buddy 返回的分配；空分配会被忽略。
         * @warning 重复释放、范围越界或释放非 Buddy 分配会触发 panic。
         */
        void put_pages(PageAllocation allocation) noexcept;
        ...
        [[nodiscard]] size_t metadata_pages() const noexcept {
            return (runtime_pool_count_ + 1) * DESCRIPTOR_POOL_PAGES;
        }

#ifndef NDEBUG
        [[nodiscard]] size_t runtime_pool_count_for_debug() const noexcept {
    ...
    private:
        ...
        static constexpr size_t DESCRIPTORS_PER_POOL    = 384;
        static constexpr size_t DESCRIPTOR_BITMAP_WORDS = DESCRIPTORS_PER_POOL / 64;
        ...
        struct DescPool {
            ...
            PageAllocation backing{};
            size_t used = 0;
            u64_t bitmap[DESCRIPTOR_BITMAP_WORDS]{};
            FreeBlock blocks[DESCRIPTORS_PER_POOL]{};
        };

        static_assert(sizeof(DescPool) <= DESCRIPTOR_POOL_PAGES * PAGE_SIZE);
        ...
        static void init_pool(DescPool &pool, PageAllocation backing) noexcept;
        void attach_pool(DescPool &pool) noexcept;
        [[nodiscard]] size_t available_descriptors() const noexcept;
        [[nodiscard]] FreeBlock *allocate_from_pool(DescPool &pool) noexcept;
        [[nodiscard]] FreeBlock *allocate_descriptor() noexcept;
        void free_descriptor(FreeBlock &block) noexcept;
        void ensure_descriptor_capacity() noexcept;
    ...
    /**
     * @brief 获取全局 Buddy 对象及其 IRQ-safe 独占锁。
     * @return 生命周期内持有本地中断保护与全局 Buddy ticket spinlock 的引用。

#!/usr/bin/env python3
"""Scan and validate library, program, and testbench metadata."""
...
KNOWN_ARCHITECTURES = ARCHITECTURES
...
def scan_host_tool_metadata_files(root: Path) -> list[Path]:
    scan_root = root / "host-tool"
    if not scan_root.is_dir():
        return []
    return sorted(scan_root.rglob("metadata.toml"))


def scan_host_tools(root: Path) -> list[HostToolMeta]:
    tools: list[HostToolMeta] = []
    seen_ids: dict[str, Path] = {}

    for metadata_path in scan_host_tool_metadata_files(root):
        with metadata_path.open("rb") as metadata_file:
            data = tomllib.load(metadata_file)

        entries = data.get("hosttool")
        if not isinstance(entries, list) or not entries:
            raise ValueError(
                f"{metadata_path}: hosttool must be a non-empty array of tables"
            )

        for entry in entries:
            if not isinstance(entry, dict):
                raise ValueError(
                    f"{metadata_path}: each hosttool entry must be a table"
                )

            tool_id = validate_global_id(entry.get("id"), f"{metadata_path}: id")
            if tool_id in seen_ids:
                raise ValueError(
                    f"duplicate host tool id {tool_id!r}: "
                    f"{seen_ids[tool_id]} and {metadata_path}"
                )
            seen_ids[tool_id] = metadata_path
            ...
            if not makefile_path.is_file():
                raise ValueError(
                    f"{metadata_path}: makefile does not exist: {makefile}"
                )
            target = entry.get("target")
            if not isinstance(target, str) or not target:
                raise ValueError(f"{metadata_path}: target must be a non-empty string")
            ...
            tools.append(
                HostToolMeta(
                    ...
                    output=normalize_output_path(
                        entry.get("output"), f"{metadata_path}: output"
                    ),
                )
            )
...
def scan_dependency_owners(root: Path) -> list[OwnerMeta]:
    ...
    for tool in scan_host_tools(root):
        if tool.id in seen_ids:
            raise ValueError(
                f"duplicate owner id {tool.id!r}: "
                f"{seen_ids[tool.id]} and {tool.metadata_path}"
            )
        seen_ids[tool.id] = tool.metadata_path
        owners.append(
            OwnerMeta(
                id=tool.id,
                root=tool.root,

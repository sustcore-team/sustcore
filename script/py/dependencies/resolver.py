...
def emit(
    ...
    lines = [
        generated_header(
            "script/py/resolve_deps.py", f"for {owner.id} ({environment})"
        ),
        "",
    ]
...
def main(arguments: list[str]) -> int:
    try:
        allowed = {"root", "owner", "environment", "arch", "output"}
        values = parse_key_value_arguments(
            arguments,
            allowed,
            required_keys={"root", "owner", "environment", "output"},
            non_empty_keys=allowed,
        )

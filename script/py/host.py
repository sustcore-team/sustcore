...
def parse_arguments(arguments: list[str]) -> tuple[Path, str | None]:
    ...
    for argument in arguments[1:]:
        ...
        if not separator or key not in {"output", "host-arch"} or not value:
...
def _make_value(value: str) -> str:
    if "\n" in value or "\r" in value:
        raise ValueError("Make values must not contain newlines")
    return value.replace("$", "$$").replace("#", r"\#")


def _shell_value(value: str) -> str:
    return _make_value(shlex.quote(value))


def _shell_flags(flags: Sequence[str]) -> str:
    return _make_value(shlex.join(flags))


def emit_make(validation: HostValidation) -> str:
    return "\n".join(
        (
            "# Generated by script/py/host.py. Do not edit this file directly.",
            "",
            "environment := host",
            f"host-arch := {_make_value(validation.arch)}",
            f"host-triple := {_make_value(validation.triple)}",
            ...
            f"host-clang-version := {_make_value(validation.clang_version)}",
            f"host-clangxx-version := {_make_value(validation.clangxx_version)}",
            f"host-llvm-ar-version := {_make_value(validation.llvm_ar_version)}",
            f"host-sysroot := {_shell_value(str(validation.sysroot))}",
            "host-sysroot-flag := --sysroot=$(host-sysroot)",
            f"host-cppstdlib := {_make_value(validation.cppstdlib)}",

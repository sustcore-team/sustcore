...
from common.constants import BUILD_MODES, SANITIZERS
from metadata.registry import HostProgramMeta, scan_testbenches


VALID_KINDS = {"test", "bench", "example"}
VALID_MODES = set(BUILD_MODES)
VALID_SANITIZERS = set(SANITIZERS)
...
def parse_arguments(arguments: list[str]) -> dict[str, str]:
    ...
    values = parse_key_value_arguments(
        arguments, allowed, required_keys={"root", "kind", "mode"}
    )

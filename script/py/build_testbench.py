#!/usr/bin/env python3
"""Generate host test/benchmark and header-check dispatch metadata."""
...
import build_ctx
from libregistry import HeaderCheckMeta, HostProgramMeta, scan_testbenches
...
def emit(root: Path) -> str:
    programs, checks = scan_testbenches(root)
    lines = [
        "# Generated by script/py/build_testbench.py. Do not edit this file directly.",
        "",
    ...
    for program in programs:
        lines.extend(
            (
                ...
                f"hostprog-{program.id}-path := $(path-{'test' if program.kind == 'test' else 'bench'})/{program.output}",
                "",
...
def emit_ctx(root: Path) -> dict[str, str]:
    programs, _ = scan_testbenches(root)
    result: dict[str, str] = {}
    for program in programs:
        output_root = "$(path-test)" if program.kind == "test" else "$(path-bench)"
        result[build_ctx.hostprog_name(program.id)] = build_ctx.emit(
            program.id,
            program.root,
            f"$(path-obj)/testbench/{program.kind}/{program.id}",
            f"{output_root}/{program.output}",
        )
    return result

...
def parse_arguments(arguments: list[str]) -> str:
    ...
    for argument in arguments:
        ...
        if key in values:
            raise ValueError("duplicate argument: config")
        values[key] = value
    return values.get("config", "default")
...
def generate(config_name: str) -> None:
    ...
    include_lines.extend(f"-include $(path-cache)/{name}" for name in generated)
...
def main(arguments: list[str]) -> int:
    try:
        generate(parse_arguments(arguments))

 ...
 * @brief 实现 RISC-V SBI Remote Fence 扩展的远程缓存与地址转换栅栏调用。
 * @version 0.1.0-dev.1
 * @date 2026-08-02
 *
...
SBIRet sbi_remote_fence_i(xlen_t hart_mask, xlen_t hart_mask_base) {
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_FENCE_I, hart_mask, hart_mask_base, 0, 0, 0, 0);
}

SBIRet sbi_remote_sfence_vma(xlen_t hart_mask, xlen_t hart_mask_base, xlen_t start_addr,
                             xlen_t size) {
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_SFENCE_VMA, hart_mask, hart_mask_base, start_addr,
                     size, 0, 0);
}

SBIRet sbi_remote_sfence_vma_asid(xlen_t hart_mask, xlen_t hart_mask_base, xlen_t start_addr,
                                  xlen_t size, xlen_t asid) {
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_SFENCE_VMA_ASID, hart_mask, hart_mask_base,
...
SBIRet sbi_remote_hfence_gvma_vmid(xlen_t hart_mask, xlen_t hart_mask_base, xlen_t start_addr,
    ...
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_HFENCE_GVMA_VMID, hart_mask, hart_mask_base,
                     start_addr, size, vmid, 0);
}

SBIRet sbi_remote_hfence_gvma(xlen_t hart_mask, xlen_t hart_mask_base, xlen_t start_addr,
                              xlen_t size) {
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_HFENCE_GVMA, hart_mask, hart_mask_base, start_addr,
                     size, 0, 0);
}

SBIRet sbi_remote_hfence_vvma_asid(xlen_t hart_mask, xlen_t hart_mask_base, xlen_t start_addr,
                                   xlen_t size, xlen_t asid) {
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_HFENCE_VVMA_ASID, hart_mask, hart_mask_base,
                     start_addr, size, asid, 0);
}

SBIRet sbi_remote_hfence_vvma(xlen_t hart_mask, xlen_t hart_mask_base, xlen_t start_addr,
                              xlen_t size) {
    return sbi_ecall(SBI_EID_RFNC, SBI_REMOTE_HFENCE_VVMA, hart_mask, hart_mask_base, start_addr,
                     size, 0, 0);
}
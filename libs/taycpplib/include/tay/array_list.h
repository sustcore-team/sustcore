...
namespace tay {
    ...
    private:
        ...
        constexpr explicit array_list(empty_tag, const allocator_type& allocator) noexcept
            : allocator_(allocator) {}

        constexpr explicit array_list(empty_tag, allocator_type&& allocator) noexcept
            : allocator_(std::move(allocator)) {}

        [[noreturn]] static constexpr void panic_error(error_code error) noexcept {
            switch (error) {
                case error_code::OUT_OF_MEMORY:            tay::panic("array_list allocation failed");
                case error_code::ALLOCATION_SIZE_OVERFLOW: tay::panic("array_list size overflow");
                case error_code::OUT_OF_RANGE:             tay::panic("array_list position out of range");
                case error_code::INVALID_ARGUMENT:         tay::panic("array_list invalid argument");
                default:                                   tay::panic("array_list operation failed");
            }
        }

        static constexpr void panic_if_error(const expected<void, error_code>& result) noexcept {
            if (!result) {
        ...
        [[nodiscard]]
        constexpr expected<size_type, error_code> next_capacity(size_type required) const noexcept {
        ...
        constexpr expected<void, error_code> reallocate(size_type new_capacity) noexcept {
            ...
            if (new_capacity != 0) {
                auto allocation = allocator_traits_type::try_allocate(allocator_, new_capacity);
                if (!allocation) {
        ...
        constexpr expected<void, error_code> ensure_capacity(size_type required) noexcept {
        ...
        [[nodiscard]]
        constexpr expected<size_type, error_code> checked_add(size_type left,
                                                              size_type right) const noexcept {
        ...
        constexpr void fill_gap(size_type index, size_type count, Source&& source) noexcept {
            const size_type old_size = size_;
            for (size_type offset = 0; offset < count; ++offset) {
                ...
                if (target < old_size) {
                    ...
                    allocator_traits_type::construct(allocator_, data_ + target, source(offset));
                }
            }
        ...
        static constexpr expected<array_list, error_code> create_range(
            ...
            for (; first != last; ++first) {
                ...
                if (!appended) {
                    return expected<array_list, error_code>(unexpect, appended.error());
                }
            }
            return result;
        }
        ...
        static constexpr expected<array_list, error_code> create_move_range(
            pointer first, pointer last, const allocator_type& allocator) noexcept {
            array_list result(empty_tag{}, allocator);
            for (; first != last; ++first) {
    ...
    public:
        ...
        constexpr explicit array_list(size_type count) noexcept
            requires(has_default_allocator && std::is_nothrow_default_constructible_v<value_type>)
            : array_list(count, allocator_type{}) {}

        constexpr array_list(size_type count, const allocator_type& allocator) noexcept
        ...
        constexpr array_list(size_type count, const_reference value) noexcept
            requires(has_default_allocator && std::is_nothrow_copy_constructible_v<value_type>)
        ...
        template <class InputIt>
            requires(
                !std::is_integral_v<InputIt> && has_default_allocator &&
                std::is_nothrow_constructible_v<value_type, decltype(*std::declval<InputIt&>())>)
        ...
        template <class InputIt>
            requires(
                ...
                std::is_nothrow_constructible_v<value_type, decltype(*std::declval<InputIt&>())>)
        constexpr array_list(InputIt first, InputIt last, const allocator_type& allocator) noexcept
            : allocator_(allocator) {
        ...
        constexpr array_list(const array_list& other) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
            : allocator_(
                  allocator_traits_type::select_on_container_copy_construction(other.allocator_)) {
        ...
        constexpr array_list(const array_list& other, const allocator_type& allocator) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
            : allocator_(allocator) {
            auto created = try_create(other, allocator_);
            if (!created) {
        ...
        constexpr array_list(array_list&& other, const allocator_type& allocator) noexcept
            : allocator_(allocator) {
            if constexpr (allocator_traits_type::is_always_equal::value) {
                ...
                auto created = create_move_range(other.begin(), other.end(), allocator_);
                if (!created) {
                    panic_error(created.error());
                }
                take_storage(std::move(*created));
        ...
        static constexpr expected<array_list, error_code> try_create(size_type count) noexcept
            requires(has_default_allocator && std::is_nothrow_default_constructible_v<value_type>)
        {
            ...
            requires(std::is_nothrow_default_constructible_v<value_type>)
        {
            ...
            if (!reserved) {
                return expected<array_list, error_code>(unexpect, reserved.error());
            }
            while (result.size_ < count) {
                allocator_traits_type::construct(result.allocator_, result.data_ + result.size_);
                ++result.size_;
            }
            return result;
        }

        static constexpr expected<array_list, error_code> try_create(size_type count,
        ...
        static constexpr expected<array_list, error_code> try_create(
            size_type count, const_reference value, const allocator_type& allocator) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
        {
            array_list result(empty_tag{}, allocator);
            auto reserved = result.reserve(count);
        ...
        template <class InputIt>
            requires(
                !std::is_integral_v<InputIt> && has_default_allocator &&
                std::is_nothrow_constructible_v<value_type, decltype(*std::declval<InputIt&>())>)
        static constexpr expected<array_list, error_code> try_create(InputIt first,
        ...
        static constexpr expected<array_list, error_code> try_create(
            std::initializer_list<value_type> values, const allocator_type& allocator) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
        {
        ...
        constexpr array_list& operator=(std::initializer_list<value_type> values) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
        {
        ...
        [[nodiscard]] constexpr expected<reference, error_code> at(size_type position) noexcept {
            if (position >= size_) {
        ...
        constexpr expected<void, error_code> shrink_to_fit() noexcept {
            return size_ == capacity_ ? expected<void, error_code>{} : reallocate(size_);
        }
        ...
        constexpr expected<void, error_code> assign(size_type count, const_reference value) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
        {
            auto created = try_create(count, value, allocator_);
            if (!created) {
        ...
        constexpr expected<iterator, error_code> emplace(const_iterator position,
                                                         Args&&... args) noexcept {
            ...
            if (!final_size) {
                return expected<iterator, error_code>(unexpect, final_size.error());
            }
            value_type value(std::forward<Args>(args)...);
            auto reserved = ensure_capacity(*final_size);
            if (!reserved) {
                return expected<iterator, error_code>(unexpect, reserved.error());
            }
            shift_right(*index, 1);
            fill_gap(*index, 1,
                     [&](size_type) noexcept -> value_type&& { return std::move(value); });
            return data_ + *index;
        }
                     ...
                     std::is_nothrow_copy_assignable_v<value_type>)
        {
            ...
            fill_gap(*index, count, [&](size_type) noexcept -> const_reference { return copy; });
            return data_ + *index;
        }
        ...
        constexpr expected<iterator, error_code> insert(const_iterator position, InputIt first,
            ...
            if (!incoming) {
                return expected<iterator, error_code>(unexpect, incoming.error());
            }
            ...
            shift_right(*index, count);
            fill_gap(*index, count, [&](size_type offset) noexcept -> value_type&& {
        ...
        constexpr expected<iterator, error_code> insert(
            const_iterator position, std::initializer_list<value_type> values) noexcept
            requires(std::is_nothrow_copy_constructible_v<value_type>)
        {
        ...
        template <class... Args>
            requires(std::is_nothrow_constructible_v<value_type, Args && ...>)
        constexpr expected<reference, error_code> emplace_back(Args&&... args) noexcept {
            auto inserted = emplace(end(), std::forward<Args>(args)...);
            if (!inserted) {
                return expected<reference, error_code>(unexpect, inserted.error());
            }
            return **inserted;
        }
            ...
            requires(std::is_nothrow_copy_constructible_v<value_type>)
        {
            ...
            while (size_ < count) {
                allocator_traits_type::construct(allocator_, data_ + size_, copy);
                ++size_;
            }
            return {};
        }
    ...
    constexpr bool operator==(const array_list<T, Allocator>& left,
        ...
        for (typename array_list<T, Allocator>::size_type index = 0; index < left.size(); ++index) {
    ...
    constexpr auto operator<=>(const array_list<T, Allocator>& left,
                               const array_list<T, Allocator>& right) noexcept {
        using result_type = decltype(std::declval<const T&>() <=> std::declval<const T&>());
        const auto count  = left.size() < right.size() ? left.size() : right.size();
        for (typename array_list<T, Allocator>::size_type index = 0; index < count; ++index) {
    ...
    template <class T, class Allocator>
    constexpr expected<void, error_code> swap(array_list<T, Allocator>& left,

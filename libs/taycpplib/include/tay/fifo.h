/**
 * @file fifo.h
 * @brief Storage-policy based circular FIFO containers.
 */
...
namespace tay {
    ...
    public:
        ...
        constexpr dynamic_fifo_storage& operator=(
            dynamic_fifo_storage&& other) noexcept {
            if (this != &other) {
                release();
        ...
        constexpr ~dynamic_fifo_storage() noexcept { release(); }

        [[nodiscard]] static constexpr expected<dynamic_fifo_storage,
                                                error_code>
        try_create(size_type capacity,
            ...
            auto memory = allocator_traits<allocator_type>::try_allocate(
                result.allocator_, capacity);
            if (!memory) {
                return expected<dynamic_fifo_storage, error_code>(
    ...
    concept fifo_storage = requires(Storage& storage,
    ...
    private:
        ...
        [[nodiscard]] constexpr size_type physical(size_type logical) const
            noexcept {
        ...
        [[nodiscard]] constexpr const value_type* ptr(size_type logical) const
            noexcept {
            return storage_.data() + physical(logical);
        }
    ...
    public:
        ...
        class basic_iterator {
            ...
            using owner_type =
                std::conditional_t<Constant, const basic_fifo, basic_fifo>;
            owner_type* owner_ = nullptr;
            size_type offset_ = 0;
            constexpr basic_iterator(owner_type* owner, size_type offset)
                noexcept
        ...
        constexpr basic_fifo(basic_fifo&& other) noexcept
            ...
            : storage_(std::move(other.storage_)), head_(other.head_),
              size_(other.size_) {
        ...
        constexpr ~basic_fifo() noexcept { clear(); }

        [[nodiscard]] static constexpr expected<basic_fifo, error_code>
        try_create(size_type capacity) noexcept
            requires requires { storage_type::try_create(capacity); }
        {
            auto storage = storage_type::try_create(capacity);
            if (!storage) {
                return expected<basic_fifo, error_code>(unexpect,
                                                        storage.error());
            }
            ...
            if constexpr (storage_type::fixed_capacity) {
                ...
                auto replacement = [&]() {
                    if constexpr (requires { storage_.get_allocator(); }) {
                        return storage_type::try_create(
                            requested, storage_.get_allocator());
                    } else {
                ...
                if (!replacement) {
                    return expected<void, error_code>(
                        unexpect, replacement.error());
                }
                for (size_type i = 0; i < size_; ++i) {
                    static_cast<void>(std::construct_at(
                        replacement->data() + i, std::move(*ptr(i))));
                }
                for (size_type i = 0; i < size_; ++i) {
        ...
        constexpr expected<void, error_code> emplace(Args&&... args) noexcept {
            ...
            static_cast<void>(std::construct_at(
                ptr(size_), std::forward<Args>(args)...));
            ++size_;
            return {};
        }
        constexpr expected<void, error_code> push(const_reference value)
            noexcept
        ...
        [[nodiscard]] constexpr expected<value_type, error_code> pop() noexcept {
            if (empty()) {
                return expected<value_type, error_code>(
        ...
        [[nodiscard]] constexpr expected<const_reference, error_code> front()
            const noexcept {
            if (empty()) {
        ...
        [[nodiscard]] constexpr fifo_segments<const value_type>
        readable_segments() const noexcept {
            if (size_ == 0) {
                return {array_view<const value_type>(nullptr, 0),
                        array_view<const value_type>(nullptr, 0)};
            }
            const size_type first_count =
                size_ < capacity() - head_ ? size_ : capacity() - head_;
            return {array_view<const value_type>(storage_.data() + head_,
                                                 first_count),
                    array_view<const value_type>(storage_.data(),
                                                 size_ - first_count)};
        }
        ...
        [[nodiscard]] constexpr fifo_segments<value_type> writable_segments()
            noexcept
            requires std::is_trivially_copyable_v<value_type>
        {
            const size_type available = capacity() - size_;
            if (available == 0) {
                return {array_view<value_type>(nullptr, 0),
                        array_view<value_type>(nullptr, 0)};
            }
            ...
            return {array_view<value_type>(storage_.data() + tail, first_count),
                    array_view<value_type>(storage_.data(),
    ...
    public:
        ...
        [[nodiscard]] static constexpr expected<byte_fifo, error_code>
        try_create(size_type capacity) noexcept {
            auto created = fifo<std::byte, Allocator>::try_create(capacity);
            if (!created) {
                return expected<byte_fifo, error_code>(unexpect,
                                                        created.error());
            }
        ...
        constexpr expected<void, error_code> try_write(
            array_view<const std::byte> input) noexcept {
        ...
        constexpr size_type write_some(
            array_view<const std::byte> input) noexcept {
            const size_type count =
                input.size() < capacity() - size() ? input.size()
                                                   : capacity() - size();
            for (size_type i = 0; i < count; ++i) {
        ...
        constexpr expected<void, error_code> try_read(
            array_view<std::byte> output) noexcept {
            if (output.size() > size()) {
                return expected<void, error_code>(
                    unexpect, error_code::UNDERFLOW_ERROR);
            }
        ...
        constexpr size_type read_some(array_view<std::byte> output) noexcept {
            const size_type count = output.size() < size() ? output.size()
                                                           : size();
            for (size_type i = 0; i < count; ++i) {
                output[i] = *fifo_.pop();
            }

/**
 * @file slot_map.h
 * @brief Generational stable handles with dense value iteration.
 */
...
namespace tay {
    struct slot_map_handle {
        ...
        [[nodiscard]] friend constexpr bool operator==(
            const slot_map_handle&, const slot_map_handle&) noexcept = default;
    ...
    namespace detail {
        struct slot_map_slot {
            ...
            std::size_t next_free = std::size_t(-1);
            bool occupied = false;
            bool retired = false;
        };
    }
    ...
    class dynamic_slot_map_storage {
        ...
        using slot_allocator = typename allocator_traits_type::template
            rebind_alloc<detail::slot_map_slot>;
        using index_allocator = typename allocator_traits_type::template
            rebind_alloc<std::size_t>;
        array_list<T, Allocator> values_;
        array_list<detail::slot_map_slot, slot_allocator> slots_;
        array_list<std::size_t, index_allocator> dense_to_slot_;

    public:
        using value_type = T;
        static constexpr bool fixed_capacity = false;
        constexpr dynamic_slot_map_storage() noexcept = default;
        constexpr explicit dynamic_slot_map_storage(
            const Allocator& allocator) noexcept
            : values_(allocator), slots_(slot_allocator(allocator)),
              dense_to_slot_(index_allocator(allocator)) {}
        [[nodiscard]] constexpr auto& values() noexcept { return values_; }
        [[nodiscard]] constexpr const auto& values() const noexcept { return values_; }
        [[nodiscard]] constexpr auto& slots() noexcept { return slots_; }
        [[nodiscard]] constexpr const auto& slots() const noexcept { return slots_; }
        [[nodiscard]] constexpr auto& dense_to_slot() noexcept { return dense_to_slot_; }
        [[nodiscard]] constexpr const auto& dense_to_slot() const noexcept { return dense_to_slot_; }
    };
    ...
    class static_slot_map_storage {
        ...
        static_vector<std::size_t, N> dense_to_slot_;
    ...
    concept slot_map_storage = requires(Storage& storage,
    ...
    public:
        ...
        using const_iterator = decltype(
            std::declval<const storage_type&>().values().begin());
    ...
    private:
        ...
        [[nodiscard]] constexpr error_code capacity_error() const noexcept {
            return storage_type::fixed_capacity
    ...
    public:
        ...
        [[nodiscard]] constexpr iterator begin() noexcept { return values().begin(); }
        [[nodiscard]] constexpr iterator end() noexcept { return values().end(); }
        ...
        template <class... Args>
        [[nodiscard]] constexpr expected<handle_type, error_code> emplace(
            Args&&... args) noexcept {
            ...
            if (free_head_ != npos) {
                ...
                if (!appended) {
                    return expected<handle_type, error_code>(
                        unexpect, appended.error());
                }
            ...
            if (!value) {
                ...
                return expected<handle_type, error_code>(unexpect,
                                                          value.error());
            }
        ...
        [[nodiscard]] constexpr value_type* get(handle_type handle) noexcept {
            return contains(handle) ? &values()[slots()[handle.index].dense]
                                    : nullptr;
        }
        [[nodiscard]] constexpr const value_type* get(handle_type handle) const
            noexcept {
            return contains(handle) ? &values()[slots()[handle.index].dense]
                                    : nullptr;
        }
        [[nodiscard]] constexpr expected<value_type&, error_code> at(

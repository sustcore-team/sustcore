/**
 * @file logger.h
 * @brief Source-aware typed logging through tay::format_to.
 */
...
namespace tay {
    ...
    namespace detail {
        ...
        inline constexpr string_view logger_root_marker(
            "libs/taycpplib/include/tay/logger.h");
        inline constexpr std::size_t logger_project_root_length =
            logger_header_path.find(logger_root_marker);

        [[nodiscard]] constexpr std::size_t logger_rfind(
            string_view text, string_view pattern, std::size_t end) noexcept {
            if (end > text.size()) {
                end = text.size();
            }
            if (pattern.empty()) {
            ...
            for (std::size_t candidate = last + 1; candidate > 0; --candidate) {
                const std::size_t position = candidate - 1;
                bool matched               = true;
        ...
        [[nodiscard]] constexpr string_view logger_relative_file(
                          ...
                          logger_project_root_length == 0)
            {
                ...
                for (std::size_t index = 0; index < logger_project_root_length;
                     ++index)
                {
        ...
        [[nodiscard]] constexpr string_view logger_function_name(
            ...
            const std::size_t parenthesis = function.rfind('(');
            std::size_t end = parenthesis == string_view::npos ? function.size()
                                                               : parenthesis;
            while (end > 0 && function[end - 1] == ' ') {
                --end;
            }

            const std::size_t scope = logger_rfind(function, "::", end);
            if (scope != string_view::npos) {
                return string_view(function.data() + scope + 2,
                                   end - scope - 2);
            }

            const std::size_t operator_name =
                logger_rfind(function, "operator ", end);
            if (operator_name != string_view::npos) {
                return string_view(function.data() + operator_name,
                                   end - operator_name);
            }
    ...
    public:
        ...
        consteval logger_format_string(
            const char (&text)[N], std::source_location location =
    ...
    private:
        ...
        [[nodiscard]] static result_type add_size(std::size_t& total,
                                                  result_type part) {
            if (!part) {
                return result_type(unexpect, part.error());
            }
            if (*part > std::numeric_limits<std::size_t>::max() - total) {
                return result_type(unexpect,
                                   format_error::output_size_overflow);
            }
            total += *part;
            return total;
        }

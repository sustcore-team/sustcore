    ...
    namespace __algo {
        struct __find {
            template <std::input_iterator I, std::sentinel_for<I> S, class T,
                ...
                requires requires(Proj& proj, std::iter_reference_t<I> element, const T& value) {
            ...
            template <input_range R, class T, class Proj = std::identity>
                requires requires(Proj& proj, range_reference_t<R> element, const T& value) {
                    std::invoke(proj, element) == value;
                }
            [[nodiscard]]
        ...
        struct __find_if {
            template <std::input_iterator I, std::sentinel_for<I> S, class Pred,
                      class Proj = std::identity>
                requires requires(Pred& pred, Proj& proj, std::iter_reference_t<I> element) {
            ...
            template <input_range R, class Pred, class Proj = std::identity>
                requires requires(Pred& pred, Proj& proj, range_reference_t<R> element) {
                    std::invoke(pred, std::invoke(proj, element));
                }
            [[nodiscard]]
            constexpr range_iterator_t<R> operator()(R&& range, Pred pred, Proj proj = {}) const {
        ...
        struct __contains {
            ...
            [[nodiscard]]
            constexpr bool operator()(R&& range, const T& value, Proj proj = {}) const {
                return (*this)(begin(range), end(range), value, std::move(proj));
            }
        };

        struct __remove_if {
            template <std::forward_iterator I, std::sentinel_for<I> S, class Pred,
                      class Proj = std::identity>
            ...
            constexpr I operator()(I first, S last, Pred pred, Proj proj = {}) const {
                while (first != last && !std::invoke(pred, std::invoke(proj, *first))) {
                    ++first;
                }
                if (first == last) {
                    return first;
        ...
        struct __remove {
            ...
            constexpr I operator()(I first, S last, const T& value, Proj proj = {}) const {
                return __remove_if{}(
                    first, last, [&value](const auto& element) { return element == value; },
                    std::move(proj));
            }

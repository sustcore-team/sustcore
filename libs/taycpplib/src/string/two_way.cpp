 ...
 * @brief Two Way Search Algorithm
 * @version 0.1.0-dev.1
 * @date 2026-07-29
 *
...
namespace tay::detail {
    namespace {
        ...
        constexpr size_type byteset_words =
            (byte_value_count + bits_per_word - 1) / bits_per_word;
        ...
        [[nodiscard]]
        static ALWAYS_INLINE unsigned char character_at(
            const char* string, size_type length, size_type position) noexcept {
        ...
        static ALWAYS_INLINE bool byte_is_present(const size_type* byteset,
            ...
            return (byteset[byte / bits_per_word] &
                    (size_type(1) << (byte % bits_per_word))) != 0;
        }

        static ALWAYS_INLINE void mark_byte(size_type* byteset,
                                            unsigned char byte) noexcept {
            byteset[byte / bits_per_word] |= size_type(1)
                                             << (byte % bits_per_word);
        }

        template <__search_direction direction>
        [[nodiscard]]
        factorization maximal_suffix(const char* pattern,
            ...
            while (candidate + offset < pattern_length) {
                const unsigned char candidate_char = character_at<direction>(
                    pattern, pattern_length, candidate + offset);
                const unsigned char suffix_char = character_at<direction>(
                    pattern, pattern_length, suffix + offset);
        ...
        template <__search_direction direction>
        [[nodiscard]]
        size_type two_way_search(const char* text, size_type text_length,
                                 const char* pattern,
            ...
            const factorization normal =
                maximal_suffix<direction>(pattern, pattern_length, false);
            const factorization reversed =
                maximal_suffix<direction>(pattern, pattern_length, true);
            ...
            for (size_type index = 0; index < critical_position; ++index) {
                if (character_at<direction>(pattern, pattern_length, index) !=
                    character_at<direction>(pattern, pattern_length,
                                            index + period))
                {
                    periodic = false;
                    break;
            ...
            if (periodic) {
                ...
                const size_type right_length =
                    pattern_length - critical_position;
                period = critical_position > right_length ? critical_position
                                                          : right_length + 1;
            }
            ...
            while (position <= text_length - pattern_length) {
                const unsigned char last_byte = character_at<direction>(
                    text, text_length, position + last_pattern_index);
                ...
                size_type bad_character_shift =
                    pattern_length - shift_table[last_byte];
                ...
                size_type index =
                    critical_position > memory ? critical_position : memory;
                while (
                    index < last_pattern_index &&
                    character_at<direction>(pattern, pattern_length, index) ==
                        character_at<direction>(text, text_length,
                                                position + index))
                {
                    ++index;
                }
    ...
    template <__search_direction direction>
    std::size_t __str_two_way(const char* text, std::size_t text_length,
                              const char* pattern,
                              std::size_t pattern_length) noexcept {
        if (pattern_length == 0) {
            if constexpr (direction == __search_direction::FORWARD) {
                return 0;
            } else {
        ...
        const size_type position = two_way_search<direction>(
            text, text_length, pattern, pattern_length);
        if constexpr (direction == __search_direction::FORWARD) {
            return position;
        } else {
            return position == npos ? npos
                                    : text_length - pattern_length - position;
        }
    }

    template std::size_t __str_two_way<__search_direction::FORWARD>(
        const char*, std::size_t, const char*, std::size_t) noexcept;
    template std::size_t __str_two_way<__search_direction::BACKWARD>(
        const char*, std::size_t, const char*, std::size_t) noexcept;
}  // namespace tay::detail

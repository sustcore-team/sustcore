 ...
 * 7. VFS进程创建共享内存区域用于文件数据缓冲, 这个过程中VFS获得一个SHM能力
 * 8. VFS进程将该SHM能力派生给请求的进程
 * 9. VFS将该能力返回给请求进程
 * 10. 请求进程接收到SHM能力后, 向内核请求共享内存资源
 * 11. 内核在用户页表中映射该共享内存区域, 并将指针返回给请求进程
 * 12. 请求进程通过映射的指针访问文件数据
 * 13. 请求进程完成对文件的操作后, 通过fclose()等接口关闭文件
 * 14. VFS进程收到关闭请求后, 使该SHM能力失效, 并释放共享内存区域
 *
 ...
 * 例如, 一个进程持有指向某个目录树的FLE能力, 它可以通过该能力请求一个访问该目录树下某个文件的能力。
 * 它也可以通过向其他进程派生指向该目录树的FLE能力, 使其也能访问该目录下的所有文件。
 *
 * -------------------------------
 *
 * 进一步的, 我们要区分PCB/TCB能力与进程能力/线程能力的区别。
 * PCB能力决定了一个进程是否能够控制某个进程控制块。这决定了它是否能够操作该进程(如调度、终止等)。
 * 而进程能力决定了一个进程是否能够"看见"另一个进程, 以及与之通信(如发送消息、共享内存等)。
 * 如果一个进程不能够"看见"另一个进程, 它就无法与之通信, 也无法向其派生能力。
 * 于是, 一个进程所持有的进程能力的集合, 就是它所知道的进程的集合。
 * 也可以理解为, 在这个进程看来, 只有它所持有进程能力的那些进程是真实存在的。
...
enum {
    ...
    CAP_TYPE_FLE = 6,    // 文件能力
    CAP_TYPE_INT = 7,    // 中断能力
    CAP_TYPE_SHM = 8,    // 共享内存能力
    CAP_TYPE_PHY = 9,    // 物理内存能力
    CAP_TYPE_PRO = 10,   // 能力提供者能力
    CAP_TYPE_CUSTOM = 11 // 自定义能力
};
 ...
 * @return int 能力类型
 */
int get_cap_type(Capability cap);
 ...
 *
 * @return Capability 空能力
 */
Capability make_null_cap(Capability provider);
 ...
 */
Capability make_cap(Capability provider, void *data_ptr);
 ...
 * @return Capability 派生后的能力
 */
Capability derive_cap(Capability parent, Capability pid);
 ...
 * @param cap 能力
 */
void unvalid_cap(Capability cap);